...
✔️ Качественные и целевые заявки по дизайну.
➖➖➖➖➖➖➖➖
...
Каждый день в бот поступает 20+ целевых заявок по дизайну и это не предел, мы стараемся постоянно находить новые чаты и увеличивать количество заявок.
...
Продли свою подписку и получи одну неделю использования в подарок⚡️
...
Пользователей с бесплатной подпиской - {REPLACE_SYMBOLS_1}
Пользователей с платной подпиской - {REPLACE_SYMBOLS_2}
...
design = ["дизайнер", "дизайн", CATEGORY, "геймдизайнер", "вебдизайнер"]
design_phrases = []

no = ['таргет', "таргетолог", "таргетинг", 'smm', 'смм', 'копирайтинг', 'копирайтер', 'seo', 'маркетолог', 'клиент', 'ассистент', 'менеджер', 'продюсер']
